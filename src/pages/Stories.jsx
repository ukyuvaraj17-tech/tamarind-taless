              ...
              {stories.map((s) => {
                ...
                return (
                  <article key={s.id}>
                    ...
                    <h2
                      onClick={() => s.story?.length > 260 && setOpenId(isOpen ? null : s.id)}
                      style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(26px,3.5vw,36px)', fontWeight: 600, color: 'var(--iv)', lineHeight: 1.25, margin: '14px 0 10px', fontVariantNumeric: 'lining-nums', cursor: 'none' }}
                    >{s.title}</h2>
