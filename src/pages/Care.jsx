...
const CARE_GUIDES = [
  {
    ...
    steps: [
      ...
      'The natural patina that develops over time is part of the artefact\'s beauty, so do not try to remove it',
    ],
  ...
  },
  {
    ...
    steps: [
      ...
      'For dust, use a very soft, dry brush, and never touch the paint surface with cloth',
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            {CARE_GUIDES.map((g, i) => (
              <div key={g.material} style={{ background: i % 2 === 0 ? 'var(--card)' : 'var(--nav)', border: '1px solid var(--line)', padding: '36px 32px' }}>
                  ...
                  <ul style={{ listStyle: 'none' }}>
                    {g.steps.map((step, j) => (
                      <li key={j} style={{ display: 'flex', gap: 12, marginBottom: 10, fontFamily: "'Cormorant Garamond', serif", fontSize: 15, color: 'rgba(106,99,80,.88)', lineHeight: 1.6 }}>
                        <span style={{ color: 'var(--gd)', flexShrink: 0, marginTop: 2, fontSize: 13 }}>✓</span>
                        <span>{step}</span>
                      </li>
                    ))}
                  </ul>
                ...
                {g.note && (
                  <div style={{ background: 'rgba(33,29,20,.06)', borderLeft: '2px solid var(--gd)', padding: '14px 18px', marginTop: 20 }}>
                    <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 15, fontStyle: 'italic', color: 'rgba(33,29,20,.8)', lineHeight: 1.7 }}>{g.note}</p>
                  </div>
                )}
              </div>
            ))}
      ...
      <section style={{ background: 'var(--card)', borderTop: '1px solid var(--line)', padding: '40px 44px', textAlign: 'center' }}>
        <div className="container" style={{ maxWidth: 500 }}>
          ...
          <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 17, fontStyle: 'italic', color: 'rgba(106,99,80,.88)', marginBottom: 22, lineHeight: 1.7 }}>
