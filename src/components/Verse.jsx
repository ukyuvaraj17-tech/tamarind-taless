...
export default function Verse({ text, attribution = 'Tamarind Taless', background = 'var(--bg)', font = 'serif' }) {
  ...
  return (
    <section style={{ background, padding: 'clamp(3.5rem,7vw,5.5rem) clamp(1.5rem,5vw,3.5rem)' }}>
      <div ref={ref} className="reveal" style={{ maxWidth: 820, margin: '0 auto', textAlign: 'center' }}>
        ...
        <blockquote style={{
          fontFamily: isSans ? "'Inter', sans-serif" : "'Cormorant Garamond', serif",
          fontSize: isSans ? 'clamp(1rem,1.8vw,1.3rem)' : 'clamp(1.25rem,2.5vw,1.8rem)',
          fontStyle: isSans ? 'normal' : 'italic',
          fontWeight: isSans ? 500 : 300,
          letterSpacing: isSans ? '.005em' : 'normal',
          color: 'var(--iv)', lineHeight: isSans ? 1.65 : 1.55, margin: '0 0 1.2rem',
        }}>
          {text}
        </blockquote>
